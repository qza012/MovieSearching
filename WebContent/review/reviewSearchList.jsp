				...
				<div class="movie_main">
					<form action="reviewSearchList" method="POST">
						<div class="search">
							<select name="search" class="searchBar">
								<option ${(search == "movieName")? "selected" : ""}
									value="movieName" selected>영화 제목</option>
								<option ${(search == "id")? "selected" : ""} value="id">사용자
									아이디</option>
								<option ${(search == "subject")? "selected" : ""}
									value="subject">리뷰 제목</option>
							</select> <input class="searchForm" id="keyword" name="keyword"
								value="${keyword}" type="text" /> <input type="button"
								id="searchButton" value="검색" onclick="keywordCheck()" />
						</div>
					</form>
					...
					<div class="paging">
						<span> <c:if test="${currPage>1}">
								<a
									href="reviewSearchList?page=${currPage-1}&search=${search}&keyword=${keyword}"><i
						...
						<span id="pageArea"> <c:forEach begin="0" end="9" var="i">
								<c:if test="${startNum+i <= maxPage}">
									<c:if test="${startNum+i != currPage}">
										<a
											href="reviewSearchList?page=${startNum+i}&search=${search}&keyword=${keyword}">${startNum+i}
										</a>
									</c:if>
									<c:if test="${startNum+i == currPage}">
										<b><a
